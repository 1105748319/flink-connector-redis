...
import org.apache.flink.connector.redis.util.MyRedisRecord;
...
//        }
    ...
    //todo 取数据，可以先从缓存拿，是否保存空结果，以防止这个key反复来查redis、导致雪崩
    public void eval(CompletableFuture<Collection<Row>> future, String key) {
        MyRedisRecord myRedisRecord = new MyRedisRecord(this.redisCommandsContainer.hget(key), this.redisCommand.getRedisDataType());
        Map<String, String> map = (Map<String, String>) myRedisRecord.getData();
        for (Map.Entry<String, String> e : map.entrySet()) {

            future.complete(Collections.singletonList(Row.of(e.getKey(),e.getValue())));
